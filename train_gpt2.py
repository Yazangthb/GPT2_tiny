...
class GPTConfig:
    block_size: int = 1024
    vocab_size: int = 50257 # 50k merges from BPE, 256bytes token + 1 byte eos
    n_layer: int = 12
    n_head: int = 12
    n_embd: int = 768
...
class DataLoaderLite:
    ...
    def next_batch(self):
        ...
        # if loading the next batch would
        # 
        #  be out of bounds, advance to next shard
...
# testing multi_gpu environment run
...
model.eval()
model.to(device)
# model = torch.compile(model) if using torch >=2.0.0
if ddp:
...
for step in range(max_steps):
    ...
    for micro_step in range(grad_accum_steps):
        ...
        with torch.autocast(device_type=device, dtype=torch.float16): # change to bfloat16 if your device supports
            logits, loss = model(x, y)
    ...
    norm = torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0) # gradiant clipping
    lr = get_lr(step)
    ...
    et = time.time()
    dt = (et - st) * 1000
    tokens_per_sec = (train_loader.B * train_loader.T * grad_accum_steps * ddp_world_size) / (et - st)
    if master_process:
        print(f'step {step:4d}: loss = {loss.item()}, lr = {lr:.4e}, norm = {norm:.4f}, time = {dt:.2f} ms, tok/sec: {tokens_per_sec:.2f}')
